...
1. Use `Crtl + F` to check if a file has `// Import sections`.
2. Locate the file. Example:
    - Go to [`ChooseBlockchain.js`](/src/Components/EN/6%20-%20How%20to%20Mint%20and%20List%20an%20NFT/ChooseBlockchain.js), use `Crtl + F` to find  `// Import sections`.
