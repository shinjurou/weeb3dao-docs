...
You can start with [this guide](/guidelines/FEDEVGUIDE.md) if you don't have prior experiences in front-end development.
...
3. Add the new page's link in [/src/routes.js ](/src/routes.js). **Example:** `"/security",`
4. Add the `Route path` for the new page in [`Index.js`](/src/Components/EN/0%20-%20Index/Index.js).
...
5. Add the new page's link for the **Navigation Bar** in [`Index.js`](/src/Components/EN/0%20-%20Index/Index.js).
...
- [How to burn your NFT](/src/Components/EN/7%20-%20Burn%20your%20NFT/BurnNFT.js)
...
- [NFT Controversy](/src/Components/EN/11%20-%20Environment%20Controversy/Environment.js)
...
3. Add attributes for the Card: `color`, `title`, `content` - `color`: Select between these options `red`, `orange`, `green`, `teal`, `purple`, `pink`. <br/>Example: `color="purple"`. - `title`, `content`: Write the title and content for the card. - For single sentence: Write within 2 `Double Quotation Marks (" ")` or `Single Quotation Marks (' ')`.<br/> **Example:** `"Due to the Blockchain's nature, this helps ensure transparency for every accounts available."` - For long paragraphs: Write the paragraph within a `HTML tag` (div, ul, etc.) that is enclosed within 2 `Brackets [ ]` and two `Parentheses ( )`.<br/> **Example:** `{[<div> <ul> <li>Transparency means no one can hide their illegal activities.</li> <li>This provides collectors the ability to find suspicious
...
> `.` means in the same folder.<br/> > `..` means go back one folder.<br/>
